...
struct rectangle *create_rectangle(struct point ul, struct point lr, 
    ...
    if (r == NULL) {
        perror("Error allocating space of rectangle");
        exit(1);
    }
    r->upperleft = ul;
    r->lowerright = lr;
    strncpy(r->label, label, sizeof(label));
    return r;
}
...
int area1(struct rectangle r) {
    ...
    int l = abs(r.upperleft.x - r.lowerright.x);
    int h = abs(r.upperleft.y - r.lowerright.y);
...
void change_label(struct rectangle *r, char *newlabel) {
    ...
    if (strlen(newlabel) < 21) {
        strncpy(r->label, newlabel, strlen(newlabel));
    }
    else {
        strncpy(r->label, newlabel, 20);
    }
}
